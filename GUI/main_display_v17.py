...
## main_display.py defines the class Ui_MainWindow to display data and run collection script
...
class Ui_MainWindow(object):
    ...
    # sets plot title based on channel name settings
    ...
    # upload_settings() method
    # uploas scope settings from external csv file       
    def upload_settings(self):
        ...
        for scope_num in range(1, NUM_SCOPE+1):  # for all scopes
            ...
            settings.setValue(f"{currentScope}/{chB_attributes[3]}", rows[3][1]) # chB_rate
    ...
    # run_collect_data_script() method
    # run the script to collect data using the PicoScope
    def run_collect_data_script(self):
        try:
            subprocess.run(["python", "C:/Users/501ALabPC/Desktop/ENPH455-JC-main/PicoScope/pico_JC/pico_scope_setup.py"], check=True)
        except subprocess.CalledProcessError as e:
            print(f"Error running script: {e}")

    # plot_data() method
    # plot data from external csv file is scope and channel selection are valid
